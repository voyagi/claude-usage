...
2. Extension accurately calculates billable vs cached token usage
3. Extension aggregates usage into time buckets (session, daily, weekly, monthly) with correct totals
4. Extension handles incomplete JSONL writes without crashing
...
5. Extension properly disposes watchers when extension deactivates
...
- Auto-learn + manual override for rate limits (Max plan limits aren't documented)
...
- Dual bundling strategy (separate Node.js/CJS extension + browser/IIFE webview)
- Streaming parser with skip-corrupt-lines resilience
- Output tokens for rate limit calculations (Claude constrains output generation)
- EMA-smoothed burn rate with configurable window
- Downward-only limit refinement from observed 429 events

**Issues Resolved:**

- Phase 1 persistence gap: loadUsageData() was defined but never called; wired in later phase for cached-first activation
- Phase 1 plans.ts orphan: getPlanConfig() was exported but unused; wired in Phase 3 via rateLimits.ts
- Phase 4 rate limit detector orphan: parseRateLimitEvent() was exported but not connected to parsing pipeline; wired in gap-closure plan 04-05
- Phase 4 weekly Sonnet tracking: Was using all-model tokens; fixed with per-model weekly aggregation in gap-closure plan 04-06

**Technical Debt Incurred:**

- paths.ts uses console.warn/error instead of Logger utility
...
- No React error boundaries in webview dashboard
- Config change listener triggers on ANY claude-usage.* change (could debounce)
- webview.js bundle is 2.0 MB (Recharts is large)
