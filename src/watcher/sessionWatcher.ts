...
import type { TimeBuckets, ModelPricing } from '../types.js';
...
export class SessionWatcher {
  ...
  start(): void {
    ...
    const changeDisposable = this.watcher.onDidChange((uri) => {
      ...
      if (this.recentlyCreated.has(filePath)) {
        logger.info(`Skipping change event for recently created file: ${path.basename(filePath)}`);
        return;
      }
   ...
   */
  private async handleFileChange(filePath: string, forceOffset?: number): Promise<void> {
    try {
      // Get offset: use forceOffset if provided, else get from tracker
      const offset = forceOffset !== undefined ? forceOffset : this.offsetTracker.getOffset(filePath);

      logger.info(`Processing file change: ${path.basename(filePath)} from offset ${offset}`);
