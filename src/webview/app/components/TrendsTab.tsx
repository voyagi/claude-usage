...
import { DashboardData, TrendDataPoint } from '../types';
...
export function TrendsTab({ data }: TrendsTabProps) {
  ...
  const totalCost = data.trendData.reduce((sum, point) => sum + point.totalCost, 0);
  const avgCost = data.trendData.length > 0 ? totalCost / data.trendData.length : 0;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ margin: 0, fontSize: 'calc(var(--vscode-font-size) * 1.2)', fontWeight: 600 }}>
          Usage Trends
        </h2>
        <SegmentedControl
          options={[
      ...
      >
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 'calc(var(--vscode-font-size) * 0.85)', color: 'var(--vscode-descriptionForeground)' }}>
            Total Cost
          </div>
          <div style={{ fontSize: 'calc(var(--vscode-font-size) * 1.4)', fontWeight: 600, marginTop: '4px' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 'calc(var(--vscode-font-size) * 0.85)', color: 'var(--vscode-descriptionForeground)' }}>
            Average per {period === 'daily' ? 'Day' : period === 'weekly' ? 'Week' : 'Month'}
          </div>
          <div style={{ fontSize: 'calc(var(--vscode-font-size) * 1.4)', fontWeight: 600, marginTop: '4px' }}>
            ...
            >
              <thead>
                <tr style={{ borderBottom: '1px solid var(--vscode-panel-border)' }}>
                  ...
                  <th style={{ padding: '8px', textAlign: 'right', fontWeight: 600 }}>Cache Write</th>
                  <th style={{ padding: '8px', textAlign: 'right', fontWeight: 600 }}>Cache Read</th>
              ...
              <tbody>
                {data.trendData.map((point, index) => (
                  <tr
                    ...
                    style={{
                      background:
                        index % 2 === 0 ? 'transparent' : 'var(--vscode-list-hoverBackground)',
                      borderBottom:
                ...
                >
                  ...
                  <td style={{ padding: '8px', textAlign: 'right' }}>
                    {formatNumber(
                      data.trendData.reduce((sum, p) => sum + p.outputTokens, 0)
                    )}
                  </td>
                  <td style={{ padding: '8px', textAlign: 'right' }}>
                    {formatNumber(
                      data.trendData.reduce((sum, p) => sum + p.cacheCreationTokens, 0)
                    )}
                  </td>
                  <td style={{ padding: '8px', textAlign: 'right' }}>
                    {formatNumber(
                      data.trendData.reduce((sum, p) => sum + p.cacheReadTokens, 0)
                    )}
                  </td>
                  <td style={{ padding: '8px', textAlign: 'right' }}>
