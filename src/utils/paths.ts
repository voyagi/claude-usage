 ...
 */
export async function findAllSessionFiles(projectsDir: string): Promise<string[]> {
        ...
        for (const entry of projectEntries) {
          ...
          if (entry.isFile() && entry.name.endsWith('.jsonl')) {
            ...
            try {
              ...
              const subagentsExists = await fs.access(subagentsPath)
                .then(() => true)
                .catch(() => false);

              if (subagentsExists) {
                const subagentFiles = await fs.readdir(subagentsPath, { withFileTypes: true });
                for (const subagentFile of subagentFiles) {
                  if (subagentFile.isFile() && subagentFile.name.endsWith('.jsonl')) {
                    sessionFiles.push(path.join(subagentsPath, subagentFile.name));
                  }
                }
              }
            } catch (error) {
              // Skip unreadable subdirectories
              console.warn(`Warning: Could not read directory ${entryPath}:`, error);
            }
          }
        }
      } catch (error) {
        // Skip unreadable project directories
        console.warn(`Warning: Could not read project directory ${projectPath}:`, error);
      }
    }
  } catch (error) {
    // Projects directory doesn't exist or is unreadable
    console.error(`Error: Could not read projects directory ${projectsDir}:`, error);
    return [];
  }
