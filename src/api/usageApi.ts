...
async function getAccessToken(logger: Logger): Promise<string | null> {
  ...
  try {
    ...
    logger.warn(`Could not read credentials: ${error instanceof Error ? error.message : error}`);
    return null;
  }
}
 ...
 */
function parseWindow(raw: { utilization?: number | null; resets_at?: string | null } | null): ApiRateLimitWindow | null {
  if (!raw || raw.utilization === null || raw.utilization === undefined) {
    return null;
  }
  // API returns percentages (0-100), normalize to 0-1 fraction
  const utilization = raw.utilization > 1 ? raw.utilization / 100 : raw.utilization;
  return {
    utilization,
 ...
 */
export async function fetchApiUsage(logger: Logger): Promise<ApiUsageData | null> {
