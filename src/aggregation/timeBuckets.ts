...
import { createEmptyAggregatedUsage, addToAggregation } from '../parser/tokenCounter';
 ...
 */
export function serializeTimeBuckets(buckets: TimeBuckets): SerializedTimeBuckets {
  return {
...
export function deserializeTimeBuckets(
  ...
  return {
    ...
    daily: new Map(serialized.daily.map(([key, agg]) => [key, deserializeAgg(agg)])),
    weekly: new Map(
